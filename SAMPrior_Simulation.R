...
#Calibration---------------------------------------
SAMPrior_Cal <- function(K,p,n,q0,pi,ph,nh,clin.diff,a,b,run){
  ...
  for(j in 1:run){
    ...
    sam <- as.data.frame(samples.sam[[1]])
    Delta.mat[j,] <- apply(sam,2,post.prob.fun)
    print(j)
  }
...
# b <- 1
# 
# sam.prior <- SAMPrior_Cal(K,p,n,q0,pi,ph,nh,clin.diff,a,b,run)
# print(sam.prior)
# 
# save(sam.prior,file='SAM_Prior_Delta.RData')


# #Simulation-----------------------------
# SAMPrior_Sim <- function(pmat,n,nh,K,q0,pw,pi,run,clin.diff,a,b,samprior.delta){
#   H <- sum(nh!=0)
#   p <- pmat[1:K]
#   ph <- pmat[K+1:H]
#   hist <- as.numeric(nh>0)
...
#     samples.sam <- coda.samples(jags.fit.sam,variable.names=c('p'),n.iter=100000,silent=T)
#     sam <- as.data.frame(samples.sam[[1]])
#     pointests[i,] <- colMeans(sam)
#     hypo[i,] <- as.integer(apply(sam,2,post.prob.fun)>samprior.delta)
#     print(i)
#   }
#   perfect <- 0 
#   for(j in 1:run){
...
# 
# samprior.sim1_9 <- mclapply(list.scenarios1,SAMPrior_Sim,n,nh,K,q0,pw,pi,run,clin.diff,a,b,samprior.delta,mc.cores=10)
# save(samprior.sim1_9,file='SAMPrior_Sim1_9.RData')
# 
# samprior.sim10_18 <- mclapply(list.scenarios2,SAMPrior_Sim,n,nh,K,q0,pw,pi,run,clin.diff,a,b,samprior.delta,mc.cores=10)
# save(samprior.sim10_18,file='SAMPrior_Sim10_18.RData')
