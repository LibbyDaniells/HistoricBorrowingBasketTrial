...
adapt.fujikawa(y,n,yh,nh,tau,hist.scalar,epsilon)

#Calibration----------------------------
Fujikawa_Cal <- function(K,p,n,ph,nh,tau,hist.scalar,epsilon,q0,run){
  ...
  for(i in 1:run){
    ...
    for(k in 1:K){
      Delta.mat[i,k] <- pbeta(q0,Par[k,1],Par[k,2],lower.tail=F)
    }
  }
...
# epsilon <- 2
# 
# fujikawa <- Fujikawa_Cal(K,p,n,ph,nh,tau,hist.scalar,epsilon,q0,run)
# print(fujikawa)
# 
# save(fujikawa,file='Fujikawa_Delta.RData')



# 
# #Simulation-----------------------------
# Fujikawa_Sim <- function(pmat,K,n,nh,tau,hist.scalar,epsilon,q0,run,fujikawa.delta){
#   H <- sum(nh!=0)
#   p <- pmat[1:K]
#   ph <- pmat[K+1:H]
...
#   fujikawa <- matrix(NA,nrow=run,ncol=K)
#   for(t in 1:K){
...
#     Par <- adapt.fujikawa(y,n,yh,nh,tau,hist.scalar,epsilon)$Parameters
#     for(k in 1:K){
#       fujikawa[i,k] <- pbeta(q0,Par[k,1],Par[k,2],lower.tail=F)
#     }
#     hypo[i,] <- as.integer(apply(fujikawa,2,post.prob.fun)>fujikawa.delta)
#     pointests[i,] <- colMeans(fujikawa)
#     print(i)
#   }
#   perfect <- 0 
#   for(j in 1:run){
...
# 
# fujikawa.sim1_9 <- mclapply(list.scenarios1,Fujikawa_Sim,K,n,nh,tau,hist.scalar,epsilon,q0,run,fujikawa.delta,mc.cores=10)
# save(fujikawa.sim1_9 ,file='Fujikawa_Sim1_9.RData')
# 
# fujikawa.sim10_18 <- mclapply(list.scenarios1,Fujikawa_Sim,K,n,nh,tau,hist.scalar,epsilon,q0,run,fujikawa.delta,mc.cores=10)
# save(fujikawa.sim10_18 ,file='Fujikawa_Sim10_18.RData')
